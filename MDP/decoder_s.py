...
for i in range(len(maze)) :
	for j in range(len(maze[0])) :
		...
		if up >= 0 :                                                     # 0 -> up 1 -> down 2 -> left 3 -> right
			transitions[x][1][down][1] = (1-p)/movez
			transitions[x][1][down][0] = 1.0*(up == end)
		...
		if up >= 0 :                                                     # 0 -> up 1 -> down 2 -> left 3 -> right
			transitions[x][2][left][1] = (1-p)/movez
			transitions[x][2][left][0] = 1.0*(up == end)
		...
		if up >= 0 :                                                     # 0 -> up 1 -> down 2 -> left 3 -> right
			transitions[x][3][right][1] = (1-p)/movez
			transitions[x][3][right][0] = 1.0*(up == end)
...
while s != end :
	...
	if optimal_policy[s] == 0 :
		...
		if right >= 0 :
			prob[3] = ((1-p)/movez)
		a = np.random.choice([0,1,2,3],p=prob)
		s = lll[s][a]
		moves.append(move[a])
