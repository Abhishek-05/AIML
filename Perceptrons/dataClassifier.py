    ...
    for i in range(1,49) :
        for j in range(1,49) :
            if getPixelVal(i,j) == 1 :
                if getPixelVal(i,j+1) == 0 or getPixelVal(i,j-1) == 0 :
                    if getPixelVal(i+1,j+1) == 1 :
                        x += 1.414213562
                    else :
                        x += 1
                    if getPixelVal(i+1,j-1) == 1 :
                    ...
                    if getPixelVal(i+1,j-1) == 1 :
                        ...
                        x += 1
                elif getPixelVal(i+1,j) == 0 or getPixelVal(i-1,j) == 0 :
                    x += 1

    features = util.Counter()
    
    features[0] = x # feature zero is the perimeter

    ########## finding area ##################

    #features[1] = datum.count(1) # area is not added because it causes high variation in data points

    features[1] = (x*x) / datum.count(1) # feature 1 is the ratio of permeter squared divided 
                                         # by area which turns out to be constant irrespective of the rotation

    ######### finding number of pixels in a random frame #######
